...
* Create a folder (e.g. mindset) and set the working directory to that folder on your device.
...
table_whatif <- data.frame(
  Item = c("Advertising", "Promotion" ,"Awareness", "Consideration", "Liking", "Sales"), 
  Start = c(100, 100, round(mean(data_shampoo$awareness)/100,3), 
    round(mean(data_shampoo$consideration)/100,3), round(mean(data_shampoo$liking_transformed),3), round(mean(data_shampoo$vol),3)), 
  New = c(500, 100, "?" ,"?" ,"?" ,"?" ), 
  Gain = c(400, 0, "?" ,"?" ,"?" ,"?"  ), 
  LRGain = c( "/", "/", "?" ,"?" ,"?" ,"?" ), 
  Contribution =c("/", "/", "?", "?", "?", "/"))
