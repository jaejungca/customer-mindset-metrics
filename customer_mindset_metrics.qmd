...
-   Clone GitHub repository of this chapter at [https://github.com/jaejungca/customer-mindset-metrics](https://github.com/jaejungca/customer-mindset-metrics) to your local machine under the folder you just created. 

-   Download the data set needed for this R application onto your working directory. The data set for this application, *mindset_shampoo.xlsx* is available on the website of this book.

-   Load the data set to R for analysis as we proceed.

In order to load the data sets to R, we need to run only a **part** of the following chunk of code, depending on the **operating system** of your device. To avoid running code that is not applicable and receiving error messages, please add ‘#’ before each line of inapplicable code in the chunk below. For example, if you are using Windows, you need to put ’#’ before *‘setwd("\~/Downloads/mindset")’*. See below.
...
\[ Y_t= cY\_{t-1}\gamma X\_{1t}{\beta*1} X*{2t}\^{\beta\_2} e\^{u_t} \]
...
\[ log(Y_t)= c'+\gamma log(Y\_{t-1}) + {\beta*1} log(X*{1t}) + {\beta*2} log(X*{2t}) + {u_t} \]
...
</br> An attitudinal metric would be meaningless for a brand if it does not impact sales. That is why we need to evaluate its power in terms of changing sales. Conversion is the degree to which movements in the attitudinal metric convert to sales, similar to a conversion rate of leads into customer orders in B2B. To calculate conversion, we again allow for a multiplicative funnel model that can be applied across different conditions.
...
\[ S_t= cS\_{t-1}\gamma A\_{t}{\beta*1} C*{t}^{^\beta*2} L*{t}{\beta\_3} e\^{v_t} \]
...
\[ Appeal = Potential \* Reponsiveness\_{short-run}\* 1/(1-Stickiness) \* Conversion \]
...
1.  First, derive the new level of each mindset metric and sales based on the changes that we decide to make to the marketing mix (i.e. advertising). </br>
...
\[New\_{M_i} = Start\_{M_i} \*(Advertising\_{new}/Advertising\_{old})\^{Responsiveness\_{Advertising\_{M_i}}})\]
...
</br> We derive the gain (loss if negative) of each mindset metric and sales by simply calculating the growth rate.
...
\[LRGain\_{M_i} = Gain\_{M_i}/(1-Carryover\_{M_i}) \]
...
\[Contribution\_{M_i} = LRGain\_{M_i} \* Conversion\_{M_i}\]
...
\[ New\_{M_i} = Start\_{M_i} \*(Promotion\_{new}/Promotion\_{old})\^{Responsiveness\_{Promo\_{M_i}}}) \]
